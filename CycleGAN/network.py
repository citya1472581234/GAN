...
class resnet_block(nn.Module):
    def __init__(self, channel, kernel, stride, padding):
        ...
        # every image indivisually norm  
        # batchnorm is full batch norm
    ...
    def forward(self, input):
        x = F.pad(input, (self.padding, self.padding, self.padding, self.padding), 'reflect')  # let input and x same size
        x = F.relu(self.conv1_norm(self.conv1(x)))
