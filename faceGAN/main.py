# -*- coding: utf-8 -*-
"""
Created on Wed Oct 17 09:56:43 2018
...
num_epochs = 50
data_dir = './data'
save_dir = './LFW_results/'
save_dir_loss = './results'
...
def plot_loss(d_losses, g_losses, num_epoch, save=False, save_dir='results/', show=False):
    ...
    if save:
        if not os.path.exists(save_dir):
            os.mkdir(save_dir)
        save_fn = save_dir + 'LFW_losses_epoch_{:d}'.format(num_epoch + 1) + '.png'
        plt.savefig(save_fn)
...
def plot_result(generator, noise, num_epoch, save=False, save_dir='LFW_results/', show=False, fig_size=(5, 5)):
    generator.eval()
    ...
    if save:
        if not os.path.exists(save_dir):
            os.mkdir(save_dir)
        save_fn = save_dir + 'LFW_epoch_{:d}'.format(num_epoch+1) + '.png'
        plt.savefig(save_fn)
...
scheduler = torch.optim.lr_scheduler.MultiStepLR(D_optimizer, milestones=[0.5 * num_epochs, 0.75 * num_epochs],gamma=0.5)
scheduler = torch.optim.lr_scheduler.MultiStepLR(G_optimizer, milestones=[0.5 * num_epochs, 0.75 * num_epochs],gamma=0.5)
# Training GAN
D_avg_losses = []
G_avg_losses = []
...
for epoch in range(num_epochs):
    ...
    save_fn1 = save_dir_loss + 'LFW_losses_epoch_{:d}'.format(epoch + 1) + '.png'
    loss_plots.append(imageio.imread(save_fn1))

    save_fn2 = save_dir + 'LFW_epoch_{:d}'.format(epoch + 1) + '.png'
    gen_image_plots.append(imageio.imread(save_fn2))

imageio.mimsave(save_dir_loss + 'LFW_losses_epochs_{:d}'.format(num_epochs) + '.gif', loss_plots, fps=5)
imageio.mimsave(save_dir + 'LFW_epochs_{:d}'.format(num_epochs) + '.gif', gen_image_plots, fps=5)